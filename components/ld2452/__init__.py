"""ESPHome component for HLK-LD2452 24 GHz multi-target tracking radar."""
...
ld2452_ns          = cg.esphome_ns.namespace("ld2452")
LD2452Component    = ld2452_ns.class_(
    "LD2452Component", cg.Component, uart.UARTDevice
)
...
CONFIG_SCHEMA = (
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(LD2452Component),
...
async def to_code(config):
    ...
    if CONF_FACTORY_RESET in config:
        ...
        cg.add(btn.set_button_type(cg.RawExpression("esphome::ld2452::LD2452Button::FACTORY_RESET")))
    ...
    if CONF_RESTART in config:
        ...
        cg.add(btn.set_parent(var))
        cg.add(btn.set_button_type(cg.RawExpression("esphome::ld2452::LD2452Button::RESTART")))
