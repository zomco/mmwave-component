...
// Commands (protocol 2.2). Every LD2410B command word has a zero high byte, so
...
void LD2410BComponent::check_uart_stale_(uint32_t now) {
...
void LD2410BComponent::write_command_frame_(uint8_t command_str, const uint8_t *command_value,
...
void LD2410BComponent::enqueue_command_(uint8_t command, const uint8_t *command_value, uint8_t command_value_len) {
...
void LD2410BComponent::service_command_queue_(uint32_t now) {
...
void LD2410BComponent::factory_reset() {
...
void LD2410BComponent::restart_module() {
  this->enqueue_command_(CMD_RESTART, nullptr, 0);
  ESP_LOGI(TAG, "Module restart queued");
}

void LD2410BComponent::query_firmware_version() { this->enqueue_command_(CMD_QUERY_FIRMWARE, nullptr, 0); }
void LD2410BComponent::query_parameters() { this->enqueue_command_(CMD_QUERY_PARAMS, nullptr, 0); }
void LD2410BComponent::query_distance_resolution() { this->enqueue_command_(CMD_QUERY_RESOLUTION, nullptr, 0); }
void LD2410BComponent::query_light_control() { this->enqueue_command_(CMD_QUERY_LIGHT_CTRL, nullptr, 0); }
void LD2410BComponent::query_noise_floor_status() { this->enqueue_command_(CMD_QUERY_NOISE_FLOOR, nullptr, 0); }

void LD2410BComponent::send_max_gate_command_() {
...
void LD2410BComponent::set_max_moving_gate(uint8_t gate) {
  this->max_moving_gate_ = std::min<uint8_t>(gate, TOTAL_GATES - 1);
  this->send_max_gate_command_();
}

void LD2410BComponent::set_max_still_gate(uint8_t gate) {
  this->max_still_gate_ = std::min<uint8_t>(gate, TOTAL_GATES - 1);
  this->send_max_gate_command_();
}

void LD2410BComponent::set_unmanned_duration(uint16_t seconds) {
  this->unmanned_duration_ = seconds;
  this->send_max_gate_command_();
}

void LD2410BComponent::set_gate_sensitivity(uint16_t gate, uint8_t move_sensitivity, uint8_t still_sensitivity) {
...
float LD2410BComponent::get_uniform_move_sensitivity() const {
...
float LD2410BComponent::get_uniform_still_sensitivity() const {
  if (!this->sensitivity_known_)
    return NAN;
  for (uint8_t i = 1; i < TOTAL_GATES; i++) {
...
void LD2410BComponent::request_distance_resolution(uint8_t index) {
...
void LD2410BComponent::send_light_control_command_() {
...
void LD2410BComponent::set_light_control_mode(uint8_t mode) {
  this->light_control_mode_ = std::min<uint8_t>(mode, 2);
  this->send_light_control_command_();
}

void LD2410BComponent::set_light_threshold(uint8_t threshold) {
  this->light_threshold_ = threshold;
  this->send_light_control_command_();
}

void LD2410BComponent::set_out_pin_level(uint8_t level) {
  this->out_pin_level_ = level ? 1 : 0;
  this->send_light_control_command_();
}

void LD2410BComponent::start_noise_floor_calibration(uint16_t seconds) {
...
void LD2410BComponent::publish_gate_sensitivity_summary_() {
...
void LD2410BComponent::handle_ack_data_() {
