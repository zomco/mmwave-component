...
def target_schema(target_id):
    return cv.Schema({
        ...
        cv.Optional("resolution"): sensor.sensor_schema(
...
CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(LD2453Component),
            # Calibration parameters
...
async def setup_target(var, config, target_idx, target_key):
    ...
    if "resolution" in t_config:
        sens = await sensor.new_sensor(t_config["resolution"])
        cg.add(var.set_target_resolution_sensor(target_idx, sens))
