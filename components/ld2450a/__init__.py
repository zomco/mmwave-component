...
ld2450a_ns = cg.esphome_ns.namespace("ld2450a")
...
CONF_GESTURE_SPEED = "gesture_speed"
CONF_GESTURE_ANGLE = "gesture_angle"
CONF_ROOM_X = "room_x"
CONF_ROOM_Y = "room_y"
CONF_ROOM_Z = "room_z"
...
CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(LD2450AComponent),
            # Calibration parameters
            cv.Optional(CONF_RADAR_X, default=0.0): cv.float_,
            cv.Optional(CONF_RADAR_Y, default=0.0): cv.float_,
            cv.Optional(CONF_RADAR_Z, default=150.0): cv.float_,
            ...
            cv.Optional(CONF_GESTURE_TYPE): sensor.sensor_schema(
                accuracy_decimals=0,
            ),
            cv.Optional(CONF_GESTURE_DISTANCE): sensor.sensor_schema(
                unit_of_measurement=UNIT_CENTIMETER,
                device_class=DEVICE_CLASS_DISTANCE,
                accuracy_decimals=1,
            ),
            cv.Optional(CONF_GESTURE_SPEED): sensor.sensor_schema(
                unit_of_measurement=UNIT_CM_PER_S,
                accuracy_decimals=0,
            ),
            cv.Optional(CONF_GESTURE_ANGLE): sensor.sensor_schema(
...
async def to_code(config):
    ...
    if CONF_GESTURE_TYPE in config:
        sens = await sensor.new_sensor(config[CONF_GESTURE_TYPE])
        cg.add(var.set_gesture_type_sensor(sens))

    if CONF_GESTURE_DISTANCE in config:
        sens = await sensor.new_sensor(config[CONF_GESTURE_DISTANCE])
        cg.add(var.set_gesture_distance_sensor(sens))

    if CONF_GESTURE_SPEED in config:
        sens = await sensor.new_sensor(config[CONF_GESTURE_SPEED])
        cg.add(var.set_gesture_speed_sensor(sens))

    if CONF_GESTURE_ANGLE in config:
        sens = await sensor.new_sensor(config[CONF_GESTURE_ANGLE])
        cg.add(var.set_gesture_angle_sensor(sens))
