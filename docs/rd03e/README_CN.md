...
`rd03e.target_timeout` 默认为 `3s`，设为 `0ms` 可禁用保持。组件先逐帧跟踪
UART 检测结果，再以 1 Hz 限制实体发布频率。短暂无目标时保留上一次检测；
距离暂时缺失时，最多保留该超时范围内的上一次实测距离。持续缺失距离会报告
零距离；持续无目标会清除运动状态、距离和存在状态。UART 静默一秒后会清除
这些状态。因此，无人状态最迟在雷达最后一次检测后的超时时间加一个发布周期
内发布。这不会提高雷达的物理灵敏度，也不会在从未检测到目标时生成有人状态。

读取参数和读取固件版本按钮会自动进入并退出配置模式。DEBUG 日志包含原始
应答；参数解析兼容手册布局及本测试设备实际返回的 54 字节应答。
