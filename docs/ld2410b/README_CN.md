...
本组件将 **LD2410B** 人体存在雷达集成到 ESPHome 中。

[English Documentation](./README.md)

LD2410B 是一款一维（1D）存在与运动侦测雷达：8 个距离门，每门 0.75 m（或
...
完整示例见 [`tests/common/ld2410b.yaml`](../../tests/common/ld2410b.yaml)，出厂
固件用的就是这一套。
...
波特率没有暴露，是因为模块会掉电保存新波特率，而固件里的 `uart:` 配置不会
跟着变：写一次，ESP 就在对一个自己再也听不见的雷达说话，而且没有任何办法
通过 OTA 救回来。

蓝牙没有暴露的理由不同。关掉蓝牙并不影响串口，但会断掉厂商 App——说明书里
调参和 OTA 走的就是这条路——而恢复的办法只有串口命令，或者在 2~3 秒内把模块
连续断电上电 5 次以上。这种东西不适合做成面板上的一个开关。
