...
Hi-Link LD2420 24 GHz FMCW precise ranging radar — ESPHome component.
...
> [!NOTE]
> This component provides a simplified parser that extracts presence and distance from the LD2420's data stream, and applies our custom 1-D coordinate transformation and boundary filtering.
> To configure internal gate energy thresholds, use the official Hi-Link tools.
...
| `distance` | `sensor` | `float` | `0` ~ `800` | cm | Continuous | Line-of-sight distance from radar to target (`device_class: distance`) |
...
> [!IMPORTANT]
> The LD2420 is a 1-D ranging radar. The `room_x/y/z` coordinates are derived by projecting the measured distance along the radar's local +X axis, and applying the calibration parameters (`radar_x/y/z`, `yaw/pitch/roll`).
...
The LD2420 uses a default baud rate of **115200**, 8N1.

```yaml
uart:
  id: uart_ld2420
  tx_pin: GPIO21   # ESP32-C3 → LD2420 RX (cross-wired)
...
### Step 3: Configure the LD2420 Component
