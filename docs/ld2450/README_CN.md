...
房间 `presence` 不变。Areas 只统计界内目标。多边形写在 `Area N Polygon`；默认离开迟滞 50 cm、静止锁 15 cm/s、Occupied 确认 0.4 s、离开保持 2 s。见 [DIY_CN.md — 占位区域](../../DIY_CN.md#方案-3占位区域)。
...
Target Frame 文本传感器通过 `t` 发布同帧厘米坐标，并通过可选的平行数组 `s` 保留从零开始的 UART 原始槽位。例如 `"t":[[-82.5,769.7,0]],"s":[1]` 表示仅槽位 1 有目标，而非槽位 0 发生位移。空帧为 `"t":[],"s":[]`。v1 格式和目标元组保持不变，兼容忽略 `s` 的消费者。雷达可能复用槽位，槽位不是永久人员身份。卡片应通过 `frame_entity` 配置此传感器。

共享 LD2450 固件显式设置 `zone_filter: {type: disabled}`。模块会掉电保存原生屏蔽区域，仅删除配置块不能清除旧区域。只有明确需要屏蔽某个区域时才应开启排除模式；此前示例会屏蔽 X = -100～100 厘米、Y = 100～500 厘米范围，导致雷达正前方的人无法被检测。
