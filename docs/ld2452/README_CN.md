...
海凌科 HLK-LD2452 24 GHz 毫米波多目标轨迹跟踪雷达 — ESPHome 组件。

[English Documentation](./README.md)

> [!IMPORTANT]
> LD2452 的说明书**未定义任何配置命令协议**，仅定义了数据上报帧。实测该模块对
> `0x00FF`（使能配置）与 `0x00A4`（蓝牙）不回 ACK，但对 `0x0090`（多目标）与
> `0x00FE`（结束配置）有应答。因此本组件只暴露实测可用的控制项：没有蓝牙开关、
> 没有 MAC 查询，也没有雷达侧区域过滤。边界过滤全部在 ESP 端通过 `polygon` 完成。
...
```
tests/
├── ld2452-esp32c3.yaml            ← 基础配置（硬件参数 + 所有传感器）
...
LD2452 采用固定的 **9600** 波特率（8N1）。此设置不可更改。
...
| `radar_z` | `float` | cm | `150.0` | 雷达距地面的安装高度（推荐 150-200cm，挂壁安装） |
