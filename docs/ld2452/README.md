...
HLK-ld2452 24 GHz mmWave multi-target tracking radar �?ESPHome component.
...
```
tests/
├── ld2452-esp32c3.yaml            �?Base config (hardware + all sensors)
└── ld2452-esp32c3.factory.yaml    �?Factory config (!include base + OTA/provisioning)
```
...
- **Factory config** (`ld2452-esp32c3.factory.yaml`): Includes the base config via `!include` and adds BLE provisioning, HTTP OTA updates, etc. Built automatically by CI �?**users typically do not need to modify this.**
...
uart:
  id: uart_ld2452
  tx_pin: GPIO21   # ESP32-C3 �?ld2452 RX (cross-wired)
...
| `yaw` | `float` | degrees | `0.0` | Yaw angle �?horizontal offset of radar forward direction relative to room Y axis, clockwise positive (�?80 ~ 180) |
| `pitch` | `float` | degrees | `0.0` | Pitch angle �?forward tilt positive (�?0 ~ 90) |
| `roll` | `float` | degrees | `0.0` | Roll angle �?right tilt positive (�?0 ~ 90) |
